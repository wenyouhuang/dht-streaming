...
public class DHT extends DHTBase implements IDHTResource, IDHTNode, IDHTBackground {
	...
	/*
	 * This version gets the local successor from routing tables.b
	 */
	// WebMethod
...
//		routing.joinCheck();
	...
	private EventListener changeBindingListener() {
		return new EventListener() {
	        public void onEvent(InboundEvent ev) {
	            try {
	            	String key = ev.readData(String.class);
	...
	private EventListener additionBindingListener(final String key) {
		return new EventListener() {
	        public void onEvent(InboundEvent ev) {
	            try {
	            	System.out.println(String.format("** Binding event (%s): %s -> %s", ev.getName(), key, ev.readData(String.class)));
	            } catch (Exception e) {
