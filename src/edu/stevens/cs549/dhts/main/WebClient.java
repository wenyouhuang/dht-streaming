...
public class WebClient {
	...
	public NodeInfo getSucc(NodeInfo n) throws DHTBase.Failed{
		URI succPath = UriBuilder.fromUri(n.addr).path("succ").build();
	...
	public NodeInfo closestPrecedingFinger(NodeInfo n, int id) throws Failed {
		// TODO Auto-generated method stub
		UriBuilder ub = UriBuilder.fromUri(n.addr).path("findClosestPrecedingFinger");
	...
	public EventSource listenForBindings(NodeInfo node, int id, String skey) throws DHTBase.Failed {
		...
		URI listenBindUrl = UriBuilder.fromUri(node.addr).path("listen").queryParam("id", id).queryParam("key", skey).build();
		info("Server Side listenForBindings(" + listenBindUrl + ")");
		WebTarget target = listenClient.target(listenBindUrl);
	...
	public void listenOff(NodeInfo node, int id, String skey) throws DHTBase.Failed {
		// TODO listen for SSE subscription requests on http://.../dht/listen?key=<key>
		// On the service side, don't expect LT request or response headers for this request.
		URI listenOffUrl = UriBuilder.fromUri(node.addr).path("listen").queryParam("id", id).queryParam("key", skey).build();
